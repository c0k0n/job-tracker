	...
	function onTabKeydown(e: KeyboardEvent) {
		...
		switch (e.key) {
			case 'Home':
				...
				break;
			case 'End':
				// Index, not an offset. This used to add `TABS.length` as a
				// delta and then wrap with `% TABS.length`, which cancelled
				// itself out — End was a no-op and silently did nothing.
				next = TABS.length - 1;
				break;
			default:
				next = (idx + (e.key === 'ArrowRight' ? 1 : -1) + TABS.length) % TABS.length;
		}
...
>
	...
	{:else}
		...
		<!--
			Every panel is always in the DOM and inactive ones carry `hidden`.
			The previous shape swapped them with an if/else-if chain, so
			`aria-controls` on the three unselected tabs pointed at ids that
			did not exist — a broken reference AT cannot resolve. Rendering
			all four costs four cheap derivations and keeps every reference
			live. `hidden` also takes the inactive panels' forms out of the
			tab order, which is the point.
		-->
		<div class="px-6 py-5">
			<div
				...
				{#if attached}
					<div>
						...
						<p class="mt-1 text-xs text-muted">
							...
							iframe, so the link above always works.
						</p>
					</div>
				{/if}
